...
describe("Deploy Contracts", () => {
  ...
  test("Burn great amount of inscription", async () => {
    ...
    await updateStakeTime(start.toString(), end.toString())
    const [, error] = await burnInscription(user, 500, true)
    ...
    await updateStakeTime(start.toString(), end.toString())
    const [, error2] = await burnInscription(user, 500, false)
    expect(error2).toContain(`Can't burn after staking end time.`);
  });
